...
public class Car
{
     ...
     * If parameter isBusinessClass is true, the car is a business-class car. 
     * If parameter isBusinessClass is false, the car is an economy-class car.
     */
     ...
     * 
     * @return true if the booking was succesful, false if not.
     */
    public boolean bookNextSeat()
    {
        ...
        for(int i = 0; i < seats.length; i++) // iterate through each seat
        {
            if(seats[i].book()) // book seat
            {
                printTicket(seats[i].number()); // print seat
     ...
     * 
     * @param seatNo The integer identifier of the seat.
     * @return true if the cancellation was succesful, false if not.
     */
    public boolean cancelSeat(int seatNo)
    {
        if((seatNo >= 1) && (seatNo <= seats.length)) // make sure seat is valid
        {
            if(seats[seatNo-1].cancelBooking()) // cancel reservation
            {
                return true;
            }
        }
        return false; // otherwise return false, booking is not cancelled
    }    
}
